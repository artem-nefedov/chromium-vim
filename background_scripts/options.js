...
// Keep the in-memory `settings` in sync when they change in storage. Scope this
// to the 'local' area and the 'settings' key specifically: MV3 fires
// storage.onChanged for EVERY area, and Phase 2 added frequent session-area
// writes (ActiveTabs/TabHistory/tabCreationOrder/popupActive) plus a local
// `commandHistory` write. The old MV2 logic ("any change that isn't `sessions`
// must be `settings`") would treat each of those as a settings reset and clobber
// the user's .cvimrc-derived config on every tab switch.
chrome.storage.onChanged.addListener(function(changes, areaName) {
  if (areaName === storageMethod && changes.settings) {
